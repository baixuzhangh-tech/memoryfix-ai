...
export default function AdminPipelineConfigurator({
  ...
  function addPipeline() {
    const firstStage = availableStageDefinitions[0]?.type || 'fal'
    ...
    updatePipelines(current => [
      ...current,
      {
        ...
        name: 'New pipeline',
        stages: [{ id: createLocalId('stage'), type: firstStage }],
      },
    ])
  }
      ...
      current.map(pipeline =>
        pipeline.id !== pipelineId
          ...
          : {
              ...
              stages: [
                ...pipeline.stages,
                { id: createLocalId('stage'), type: firstStage },
              ],
            }
      )
    )
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-3xl font-black tracking-tight text-[#211915]">
            Configure model order without editing code.
          </h2>
          <p className="mt-3 max-w-3xl leading-7 text-[#66574d]">
            Build one-stage or multi-stage pipelines like `fal`, `CodeFormer
            only`, or `fal + CodeFormer`. The default pipeline is used when you
            run AI without choosing a specific pipeline.
          </p>
        </div>
          ...
          >
            ...
            <div className="mt-4 grid gap-3">
              {pipeline.stages.map((stage, stageIndex) => {
                const matchedDefinition = stageDefinitions.find(
                  definition => definition.type === stage.type
                )
                const currentDefinition =
                  matchedDefinition || availableStageDefinitions[0]

                return (
                  <div
                    key={stage.id}
                    className="grid gap-3 rounded-[1.25rem] border border-[#e6d2b7] bg-white p-4 lg:grid-cols-[1.2fr_1fr_auto]"
                  >
                    <div className="grid gap-2">
                      <span className="text-xs font-black uppercase tracking-[0.14em] text-[#9b6b3c]">
                        Stage {stageIndex + 1}
